...
(function () {
	...
	Chart.prototype._plotAreaMouseUp = function (x, y) {
		...
		if (this.plotInfo.axisPlacement === "normal" || this.plotInfo.axisPlacement === "xySwapped") {
			if (this.isDrag) {
				...
				//可以修改为点击才出现tooltip
				if (this._toolTip.enabled && this._toolTip._options.clickShow && !this.panEnabled && !this.zoomEnabled) {
	...
	Chart.prototype._plotAreaMouseMove = function (x, y) {
		...
		} else if (this._toolTip.enabled && !this._toolTip._options.clickShow)
			//标准移动出现tooltip
			this._toolTip.mouseMoveHandler(x, y);
	}
