...
define([
    ...
    "lib/harViewer/preview/jsonSchema",
    "lib/harViewer/preview/ref",
    "lib/harViewer/preview/harSchema",
    "lib/harViewer/core/cookies",
    "lib/harViewer/core/trace",
    "lib/harViewer/jquery-plugins/jquery.json"
],
...
HarModel.validateRequestTimings = function(input)
{
    ...
    for (var i=0; i<entries.length; i++)
    {
        ...
        if (total != entry.time)
        {
            var message = Lib.formatString("Sum of request timings doesn't correspond to the total value: " +
        ...
        if (timings.blocked < -1 ||
            ...
            timings.wait < -1)
        {
            var message = Lib.formatString("Negative time is not allowed: " +
